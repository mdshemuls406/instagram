...
"""
Instagram Premium Bot (Time & Account Limit)
Features:
- Multi-User Concurrency
- Persistent Database (Saves Time & Limit)
- Admin Commands: /access ID Time Limit
...
class Config:
    ...
    CHANNEL_LINK = "https://t.me/+Tp0Xq44i7_c3ZTJl"
    
    SESSION_NAME = "ig_premium_bot_limit"
    WORKDIR = Path("ig_data")
    WORKDIR.mkdir(exist_ok=True)
    MAX_THREADS = 20
    DB_FILE = "premium_users_limit.db"
...
def init_db():
    ...
    c = conn.cursor()
    # Create table with limit column
    c.execute('''CREATE TABLE IF NOT EXISTS access_users 
                 (user_id INTEGER PRIMARY KEY, expiry_time REAL, account_limit INTEGER)''')
    
    # Migration: Add account_limit column if it doesn't exist (for old DBs)
    try:
        c.execute("ALTER TABLE access_users ADD COLUMN account_limit INTEGER DEFAULT 100")
    except:
...
def set_access(user_id, seconds, limit):
    expiry = time.time() + seconds
    conn = sqlite3.connect(Config.DB_FILE)
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO access_users VALUES (?, ?, ?)", (user_id, expiry, limit))
    conn.commit()
    conn.close()
...
def check_access(user_id):
    ...
    conn = sqlite3.connect(Config.DB_FILE)
    c = conn.cursor()
    c.execute("SELECT expiry_time, account_limit FROM access_users WHERE user_id=?", (user_id,))
    result = c.fetchone()
    conn.close()
    ...
    limit = result[1] if len(result) > 1 else 100 # Default if null
...
def get_all_users_info():
    conn = sqlite3.connect(Config.DB_FILE)
    c = conn.cursor()
    c.execute("SELECT user_id, expiry_time, account_limit FROM access_users")
    data = c.fetchall()
    conn.close()
    return data
...
async def check_permissions(client, user_id):
    # Returns: isValid, StatusText, AccountLimit
    if user_id == Config.ADMIN_ID: return True, "Admin", 999999
...
async def give_access(client, message):
    try:
        ...
        set_access(target_id, seconds, limit)
        ...
        await message.reply_text(f"❌ Error: Invalid Format\n{e}")
...
# --- 3. TEXT HANDLER (UPDATED WITH LIMIT CHECK) ---
