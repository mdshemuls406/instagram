...
"""
Instagram Premium Bot (GitHub Sync + Verify + Proxy Switch)
Features:
- /on proxies & /off proxies Command
- Dynamic Thread Control (5 vs 20)
- Verify Button for Channel Join
- Permanent Database on GitHub
"""
...
class Config:
    ...
    SESSION_NAME = "ig_premium_bot_verify"
...
def process_account(u, p, k):
    ...
    # UPDATED: Check GlobalState before injecting proxy
...
async def start(client, message):
    ...
    if is_valid:
        ...
        proxy_mode = "✅ ON" if GlobalState.USE_PROXY else "❌ OFF"
        ...
        await message.reply_text(
            ...
            f"{expiry_text}\n"
            f"{limit_text}\n"
            f"🌐 **Proxies:** `{len(PROXY_LIST)}` ({proxy_mode})\n"
...
async def verify_callback(client, callback_query):
    ...
    if is_valid:
        ...
        await callback_query.answer("❌ You are not in the channel yet!", show_alert=True)
    else:
        await callback_query.answer("❌ You joined, but your access is EXPIRED.", show_alert=True)

# --- 3. ADMIN COMMANDS (UPDATED FOR PROXY SWITCH) ---

@app.on_message(filters.command(["on", "off"]) & filters.user(Config.ADMIN_ID))
async def proxy_switch_handler(client, message):
    try:
        ...
        if target.lower() == "proxies":
            if cmd == "off":
                GlobalState.USE_PROXY = False
                GlobalState.THREADS = 5
                await message.reply("🚫 **Proxies Disabled**\n⬇️ Threads reduced to 5 (Safe Mode)")
            else:
                GlobalState.USE_PROXY = True
                GlobalState.THREADS = 20
                await message.reply("✅ **Proxies Enabled**\n⬆️ Threads set to 20 (Fast Mode)")
        else:
            await message.reply("⚠️ Usage: `/on proxies` or `/off proxies`")
...
async def handle_files(client, message):
    ...
    if not PROXY_LIST and GlobalState.USE_PROXY:
        await message.reply_text("⚠️ **Proxies Offline!** Contact Admin.")
        return

    # Check Speed Status
    mode = "Fast (20 Threads)" if GlobalState.THREADS == 20 else "Safe (5 Threads)"
    p_status = "✅ ON" if GlobalState.USE_PROXY else "❌ OFF"
    ...
    # UPDATED: Use dynamic thread count from GlobalState
