...
"""
Instagram Premium Bot V10 (Single Thread Super Hardcore)
Features:
- Single Thread Only (1 Account at a time)
- Super Hardcore Login: 10 Retries per account
- Rotates IP & UserAgent on every retry
- GitHub Persistence & Admin Controls
"""
...
# Global State: Always 1 Thread now
class GlobalState:
    USE_PROXY = True
    THREADS = 1  # Changed to 1 as requested
...
def get_headers(mobile=False):
    if mobile:
        # Rotating Mobile UA
        return {
            'User-Agent': f'Instagram 269.0.0.18.75 Android ({random.randint(24,30)}/{random.randint(8,12)}.0.0; 480dpi; 1080x1920; Samsung; SM-G973F; beyond1; exynos9820; en_US; {random.randint(300000000, 400000000)})',
...
def attempt_login(u, p, k, use_mobile_ua=False):
    ...
    # Rotate Proxy on every attempt if enabled
...
def process_account_hardcore(u, p, k):
    ...
    for i in range(10):
        ...
        # Stop only on fatal errors, otherwise keep retrying with new IP
...
async def start(client, message):
    ...
    if is_valid:
        ...
        await message.reply_text(
            f"💠 **IG PREMIUM BOT V10** 💠\n\n"
            f"👋 Hello, **{message.from_user.first_name}**\n"
            f"🛡️ Status: `Active`\n"
            f"⏳ Expiry: `{status}`\n"
            f"🔢 Limit: `{limit}`\n"
            f"🌐 Proxies: `{len(PROXY_LIST)}` ({pmode})\n\n"
            f"⚡ **Mode:** Single Thread (10 Retries)\n"
...
async def switch(c, m):
    if "proxies" in m.text:
        ...
        # CHANGED: Always keep threads to 1
        GlobalState.THREADS = 1
...
async def handle(c, m):
    ...
    # CHANGED: 10 Retries text
    msg = await m.reply(f"💎 **Processing...**\nTarget: `{len(lines)}`\nMode: Super Hardcore (10x Retry)")
...
if __name__ == "__main__":
    print("🔥 Hardcore Bot V10 Started...")
    app.run()
