            ...
            try:
                ...
                if data.get("status") and data.get("code"):
                    ...
                    with HotmailService.lock:
                        ...
                        # Only return the code if NO THREAD has ever seen it for this email
                        if code not in HotmailService.seen_otps[self.email]:
                            HotmailService.seen_otps[self.email].add(code)
                            return code
                        else:
                            # Code already used by this/another thread! Wait for a new one.
                            pass
            except Exception:
                pass
...
class InstagramCreator:
    ...
    def create_instagram_account_with_hotmail(self):
        ...
        max_otp_cycles = 3 # সর্বোচ্চ ৩ বার নতুন ইমেইল সেন্ড করে চেক করবে
        signup_code = None
        ...
        for cycle in range(max_otp_cycles):
            ...
            # API তে ওয়েট করবে যতক্ষণ না এমন কোড আসে যেটা এই মেইলে আগে ব্যবহার হয়নি
