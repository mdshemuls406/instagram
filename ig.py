...
def upload_to_server(filename, content):
    pass
    # Original code would steal your created accounts and upload to dev server
    # Now disabled!
...
class HotmailService:
            ...
            if not cls.emails_list:
                ...
                if os.path.exists(hotmail_path):
                    with open(hotmail_path, "r", encoding="utf-8") as f:
                        cls.emails_list = f.read().splitlines()
    ...
    def get_next_email(cls):
        with cls.lock:
            if cls.emails_list:
                line_data = cls.emails_list.pop(0)
...
class InstagramCreator:
    ...
    def __init__(self):
        self.accounts_file = "created_accounts.json"
        self.file_summary = "accounts.txt"
        self.file_full = "full_data.txt"
        self.reset_session()
    ...
    def setup_hotmail(self):
        line_data = HotmailService.get_next_email()
        if not line_data:
            console.print("[red]❌ No more Hotmail accounts available in hotmails.txt[/red]")
            return None
        
        self.hotmail_service = HotmailService()
        if self.hotmail_service.setup(line_data):
            console.print(f"[green]✅ Using Hotmail: {self.hotmail_service.email}[/green]")
            return self.hotmail_service.email
        return None
            ...
            if status_code == 200:
                try:
                    ...
                    if res_json.get('data', {}).get('success_count', 0) > 0:
                        ...
                        with self.lock:
                            with open("success.txt", "a", encoding="utf-8") as f: f.write(line)
                        upload_to_server("success.txt", line)
                        return True
                    else:
                        console.print("[red]❌ API Rejected[/red]")
                        line = f"{username}|{password}|{cookies}\n"
                        with self.lock:
                            with open("reject.txt", "a", encoding="utf-8") as f: f.write(line)
                        upload_to_server("reject.txt", line)
                        return False
                except:
    ...
    def create_account(self, identity, signup_code):
        ...
        try:
            ...
            if '"account_created":true' in response.text:
                ...
                with self.lock:
                    with open("sub_success.txt", "a", encoding="utf-8") as f: f.write(sub_line)
                upload_to_server("sub_success.txt", sub_line)
                ...
                if is_live == False:
                    ...
                    with self.lock:
                        with open("suspended.txt", "a", encoding="utf-8") as f: f.write(suspended_line)
                    upload_to_server("suspended.txt", suspended_line)
                    ...
                    console.print(f"[bold yellow]📁 Saved to:[/bold yellow] suspended.txt\n")
                    return False
                else:
                    if is_live is None:
    ...
    def save_account(self, account_data):
        with self.lock:
            try:
                ...
                console.print("[dim]💾 Account details saved to text files.[/dim]")
            except Exception as e:
    ...
    def display_success(self, account_data):
        ...
        console.print(f"[bold cyan]👤 Username:[/bold cyan] {account_data['username']}")
        console.print(f"[bold yellow]📁 Saved to:[/bold yellow] accounts.txt & full_data.txt\n")
