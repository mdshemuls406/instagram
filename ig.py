...
class EmbuxService:
    def __init__(self, username, password):
        ...
        self.task_url = "https://www.embux.io/task/e2ac1ef5-d0ae-4c7d-b268-39ab35188635/"
        self.execute_url = "https://www.embux.io/task/e2ac1ef5-d0ae-4c7d-b268-39ab35188635/execute/"
        self.email = None
        self.baseline_codes = set()

    def login(self):
        try:
            console.print("[dim]🔄 Navigating to Embux Login Page...[/dim]")
            login_url = "https://www.embux.io/accounts/login/"
            ...
            if form:
                ...
                if action:
                    if action.startswith('http'):
                        ...
                        action_url = "https://www.embux.io" + (action if action.startswith('/') else '/' + action)

                # Capture all inputs (including hidden security tokens)
                for inp in form.find_all('input'):
                    name = inp.get('name')
                    if name:
                        if name.lower() in['login', 'username', 'email']:
                ...
                # Also grab any button values if they exist
                ...
                csrf_token = csrf.get("value") if csrf else self.session.cookies.get('csrftoken', '')
                payload = {
                    "login": self.username,
            ...
            headers = {
                "Referer": login_url,
                "Origin": "https://www.embux.io",
                "Content-Type": "application/x-www-form-urlencoded",
            }

            console.print("[dim]➡️ Submitting Login Form...[/dim]")
            r2 = self.session.post(action_url, data=payload, headers=headers, timeout=20)

            # If "Welcome back" is no longer on the page, login was successful
            if "dashboard" in r2.url.lower() or "Welcome back" not in r2.text:
                console.print(f"[green]✅ Successfully logged into Embux.io as {self.username}[/green]")
                return True
            else:
                # Attempt to extract specific error message from the page
                soup2 = BeautifulSoup(r2.text, 'html.parser')
                err_list = soup2.find('ul', class_='errorlist')
                if err_list:
                    console.print(f"[red]❌ Embux login failed: {err_list.text.strip()}[/red]")
                else:
                    console.print("[red]❌ Embux login failed. Please check your username/password in embux.txt.[/red]")
                return False

        except Exception as e:
            console.print(f"[red]❌ Embux login error: {e}[/red]")
            return False

    def reset_task(self):
        try:
            console.print("[dim]🔄 Reloading Embux Task Page for new thread...[/dim]")
            self.session.get(self.task_url, timeout=15)
            time.sleep(2)
        except Exception as e:
            pass
    ...
    def get_email(self):
        try:
            console.print("[dim]➡️ Accessing execute page to collect email...[/dim]")
            r = self.session.get(self.execute_url, timeout=15)
            soup = BeautifulSoup(r.text, 'html.parser')
            
            email_found = None
            
            # 1. Search in input fields (Web Parsing)
            for inp in soup.find_all(['input', 'textarea']):
                val = inp.get('value', '') or inp.text
                match = re.search(r'[a-zA-Z0-9._-]+@(hotmail\.com|outlook\.com)', str(val), re.IGNORECASE)
                if match:
                    email_found = match.group(0)
                    break
                    
            # 2. Search in all text elements if not found in inputs (Web Parsing)
            if not email_found:
                for text in soup.stripped_strings:
                    match = re.search(r'[a-zA-Z0-9._-]+@(hotmail\.com|outlook\.com)', str(text), re.IGNORECASE)
                    if match:
                        email_found = match.group(0)
                        break
                        
            if email_found:
                self.email = email_found
                self.baseline_codes = set(re.findall(r'\b\d{6}\b', r.text))
                console.print(f"[green]✅ Retrieved email from Embux: {self.email}[/green]")
                return self.email
            else:
                console.print("[red]❌ Could not find email on Embux execute page. Task might be unavailable or limit reached.[/red]")
                return None
        except Exception as e:
            console.print(f"[red]❌ Error fetching Embux email: {e}[/red]")
            return None

    def click_sent_email(self):
        try:
            r = self.session.get(self.execute_url, timeout=15)
            soup = BeautifulSoup(r.text, 'html.parser')
            ...
            for f in forms:
                ...
                if method == 'post':
                    action = f.get('action') or self.execute_url
                    if not action.startswith('http'):
                        action = "https://www.embux.io" + action if action.startswith('/') else self.execute_url
                        
                    data = {inp.get('name'): inp.get('value', '') for inp in f.find_all('input') if inp.get('name')}
                    self.session.post(action, data=data, headers={"Referer": self.execute_url}, timeout=15)
                    console.print("[blue]🖱️ Clicked 'I sent email, find code' button on Embux.[/blue]")
                    return True
                    
            csrf = self.session.cookies.get("csrftoken", "")
            self.session.post(self.execute_url, data={"csrfmiddlewaretoken": csrf}, headers={"Referer": self.execute_url}, timeout=15)
            console.print("[blue]🖱️ Fallback: Sent POST to execute URL.[/blue]")
            return True
        except Exception as e:
            console.print(f"[red]❌ Error clicking button on Embux: {e}[/red]")
        return False

    def poll_for_code(self):
        console.print("[blue]⏳ Waiting for OTP on Embux (approx 1-2 mins)...[/blue]")
        for i in range(25): 
            time.sleep(5)
            try:
                r = self.session.get(self.execute_url, timeout=15)
                current_codes = set(re.findall(r'\b\d{6}\b', r.text))
                new_codes = current_codes - self.baseline_codes
                if new_codes:
                    code = new_codes.pop()
                    return code
            except:
...
class InstagramCreator:
    ...
    _common_password = None
    
    # Global Embux Variables & Lock for Sequential Threading
    _embux_username = None
    _embux_password = None
    _embux_task_lock = threading.Lock()
    _embux_service_instance = None
    ...
    def setup_password_source(self):
        console.print("\n[bold cyan]  - Choose Password Type[/bold cyan]")
        print('------------------------------------------')
        console.print("[1] - Use random password")
        console.print("   [2] - Use common password")
        print('------------------------------------------')
        
        choice = Prompt.ask("- Choose", choices=["1", "2"])
        if choice == "2":
            InstagramCreator._common_password = Prompt.ask("- Input common password")
            console.print(f"[green]✅ Common password set to: {InstagramCreator._common_password}[/green]")
        else:
            InstagramCreator._common_password = None
    ...
    def generate_bangladeshi_name(self):
        first_names =[
            "Rakib", "Hasan", "Shakil", "Mehedi", "Tariqul", "Jamil", "Arif", "Tamim", "Rifat", "Sajib", 
            "Nayan", "Rubel", "Sohel", "Anis", "Sumon", "Faisal", "Akash", "Roni", "Mamun", "Faruq", 
            "Shamim", "Imran", "Kamrul", "Babul", "Nasim", "Tarek", "Sabbir", "Asif", "Shohag", "Masud",
            "Sadia", "Sumaiya", "Nipa", "Riya", "Tanjina", "Mim", "Jannat", "Nusrat", "Afsana", "Sonia", 
            "Farzana", "Tasnim", "Habiba", "Monika", "Purnima", "Bristy", "Mitu", "Khadija", "Ayesha", "Fatema", 
            "Ritu", "Tania", "Sharmin", "Moumita", "Nazmun", "Kanon", "Laboni", "Nadia", "Sanjida"
        ]
        last_names =[
            "Hasan", "Rahman", "Hossain", "Ali", "Islam", "Uddin", "Ahmed", "Khan", "Chowdhury", "Mahmud", 
            "Sikder", "Biswas", "Sheikh", "Miah", "Sarker", "Talukder", "Molla", "Kazi", "Bhuiyan", "Akter", 
            "Khatun", "Begum", "Howlader", "Mia", "Jaman", "Haque"
        ]
    ...
    def generate_password(self):
        if InstagramCreator._common_password:
            return InstagramCreator._common_password
    ...
    def create_account(self, identity, signup_code):
        ...
        first_name = self.generate_bangladeshi_name()
        ...
        try:
            ...
            if '"account_created":true' in response.text:
                ...
                # --- CHROME BROWSER FORMATTED COOKIES ---
                cookies_dict = self.st4_session.cookies.get_dict()
                
                if 'ig_did' not in cookies_dict:
                    cookies_dict['ig_did'] = self.device_id.upper()
                if 'wd' not in cookies_dict:
                    cookies_dict['wd'] = f"{random.choice([360, 390, 412, 414])}x{random.choice([767, 800, 844, 896, 915])}"
                if 'dpr' not in cookies_dict:
                    cookies_dict['dpr'] = random.choice(["1.5", "2", "2.5", "3"])
                    
                ordered_keys =["csrftoken", "datr", "ig_did", "wd", "dpr", "mid", "ds_user_id", "sessionid", "rur"]
                
                final_cookie_parts =[]
                for k in ordered_keys:
                    if k in cookies_dict:
                        val = cookies_dict[k]
                        if k == 'rur' and not str(val).startswith('"'):
                            val = f'"{val}"'
                        final_cookie_parts.append(f"{k}={val}")
                        
                for k, v in cookies_dict.items():
                    if k not in ordered_keys:
                        final_cookie_parts.append(f"{k}={v}")
                        
                cookie_string = "; ".join(final_cookie_parts)
                session_id = cookies_dict.get('sessionid', 'N/A')
                # -----------------------------------------
                ...
                if is_live == False:
                    ...
                    console.print(f"[bold cyan]👤 Name:[/bold cyan] {account_data['first_name']}")
    ...
    def create_instagram_account_with_embux(self):
        with InstagramCreator._embux_task_lock:
            console.print("\n[blue]🌐 Starting Embux.io workflow...[/blue]")
            
            is_first = False
            if not InstagramCreator._embux_service_instance:
                embux = EmbuxService(InstagramCreator._embux_username, InstagramCreator._embux_password)
                if not embux.login():
                    return False
                InstagramCreator._embux_service_instance = embux
                is_first = True
            else:
                embux = InstagramCreator._embux_service_instance
                
            if not is_first:
                embux.reset_task()
                
            email_address = embux.get_email()
            ...
            embux.click_sent_email()
            
            code = embux.poll_for_code()
            if not code:
                console.print("[red]❌ Timeout waiting for OTP from Embux[/red]")
                return False
                
            console.print(f"[green]🔑 Code retrieved from Embux: {code}[/green]")
            signup_code = self.verify_code(email_address, code)
    ...
    def process_single_account(self, index, total, email_type):
        ...
        try:
            ...
            elif email_type == "embux":
                success = creator.create_instagram_account_with_embux()
            else:
    ...
    def run(self):
        try:
            ...
            if email_type == "embux":
                embux_file = os.path.join(OUTPUT_DIR, "embux.txt")
                if not os.path.exists(embux_file):
                    try:
                        with open(embux_file, "w", encoding="utf-8") as f:
                            f.write("username password\n")
                    except: pass
                    console.print(f"\n[red]❌ embux.txt not found! A blank file has been created at {embux_file}[/red]")
                    console.print("[yellow]Please open the file, add your Embux 'username password' (separated by space) and run the script again.[/yellow]\n")
                    sys.exit()
                    
                with open(embux_file, "r", encoding="utf-8") as f:
                    content = f.read().strip()
                    parts = content.split()
                    if len(parts) >= 2:
                        InstagramCreator._embux_username = parts[0]
                        InstagramCreator._embux_password = parts[1]
                    else:
                        console.print(f"\n[red]❌ Invalid format in {embux_file}. Use 'username password'[/red]\n")
                        sys.exit()
            
            self.setup_ua_source()
            self.setup_password_source()
