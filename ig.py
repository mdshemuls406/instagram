...
class InstagramCreator:
    ...
    def follow_user_with_requests(self, session_cookies):
        # 8 Hardcoded Target Users & their Instagram User IDs
        target_users = {
            "instagram": "25025320",
            "cristiano": "173560420",
            "rohan_hossan1235": "79286763462",
            "whyhats": "9215539196", 
            "redbull": "476322",
            "khaby00": "779085683",
        }
        
        # Randomly select 2 Targets
        selected_targets = random.sample(list(target_users.items()), 2)
        ...
        for username, user_id in selected_targets:
            ...
            try:
                ...
                if response.status_code == 200 and '"status":"ok"' in response.text:
                    console.print(f"[cyan]✅ Followed @{username} Successfully[/cyan]")
                else:
                    console.print(f"[dim]⚠️ Follow request to @{username} sent, but status unknown[/dim]")
            except Exception:
                pass
    ...
    def push_to_api(self, username, password, cookies):
        api_url = "http://43.173.119.225/api/api/v1/webhook/nRlmI2-8T7x2DAWe1hWxi97qGA1FcCxrNcyCtLTO_Cw/account-push"
            ...
            if status_code == 200:
                try:
                    ...
                    if res_json.get('data', {}).get('success_count', 0) > 0:
                        ...
                        with self.lock:
                            with open(os.path.join(OUTPUT_DIR, "success.txt"), "a", encoding="utf-8") as f: 
                                f.write(line)
                            # --- ONLY SAVE SUCCESS USERNAME ---
                            with open(os.path.join(OUTPUT_DIR, "success_username.txt"), "a", encoding="utf-8") as f:
                                f.write(f"{username}\n")
                        return True
                    else:
        ...
        try:
            ...
            if '"account_created":true' in response.text:
                ...
                if is_live == False:
                    ...
                    # --- CALLING THE HARDCODED RANDOM FOLLOW SYSTEM ---
