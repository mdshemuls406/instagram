...
class InstagramCreator:
    ...
    # Global Cache for User IDs to minimize API Requests completely and bypass Rate Limit blocks
    _user_id_cache = {
        "instagram": "25025320",
        "cristiano": "173560420",
        "mrbeast": "11280389369",
        "jordi.koalitic": "2059371050",
        "whyhats": "2958611100", 
        "redbull": "21213498",
        "khaby00": "779085683",
        "philadelphiaeagles": "216853835"
    }
    ...
    def get_target_user_id(self, username, cookie_string):
        """Fetch User ID from Cache or dynamically if not in Cache."""
        ...
        try:
            ...
            # Using topsearch as it is much less rate-limited than web_profile_info
            ...
            if response.status_code == 200:
                data = response.json()
                for user_node in data.get('users',[]):
                    user_info = user_node.get('user', {})
                    if user_info.get('username') == username:
                        uid = user_info.get('pk')
                        with self._user_id_cache_lock:
                            self._user_id_cache[username] = str(uid)
                        return str(uid)
        except Exception:
            pass
    ...
    def follow_user_with_requests(self, session_cookies, user_id):
        url = f"https://www.instagram.com/api/v1/friendships/create/{user_id}/" 
        cookies = {c.split('=')[0].strip(): c.split('=')[1].strip() for c in session_cookies.split('; ') if '=' in c}
        headers = {
            "x-csrftoken": cookies.get('csrftoken', ''),  
            "x-ig-app-id": "936619743392459",  
        ...
        try:
            response = requests.post(url, headers=headers, cookies=cookies, timeout=20)
    ...
    def push_to_api(self, username, password, cookies):
        api_url = "http://43.135.182.151/api/api/v1/webhook/nRlmI2-8T7x2DAWe1hWxi97qGA1FcCxrNcyCtLTO_Cw/account-push"
    ...
    def create_account(self, identity, signup_code):
        ...
        try:
            ...
            if '"account_created":true' in response.text:
                console.print("[cyan]🌍 Hitting Instagram homepage to collect full mobile cookies...[/cyan]")
                ...
                try:
                    self.st4_session.get("https://www.instagram.com/", headers=hp_headers, timeout=20)
                except:
                    pass
                
                c = self.st4_session.cookies.get_dict()
                
                if not c.get('ds_user_id') and c.get('sessionid'):
                    decoded_session = urllib.parse.unquote(c['sessionid'])
                    c['ds_user_id'] = decoded_session.split(':')[0]
                
                if 'wd' not in c: c['wd'] = '360x767'
                if 'dpr' not in c: c['dpr'] = '2'
                if 'ig_did' not in c: c['ig_did'] = self.device_id
                if 'mid' not in c or not c['mid']: 
                    c['mid'] = "".join(random.choices(string.ascii_letters + string.digits, k=28))
                if 'datr' not in c or not c['datr']:
                    c['datr'] = "".join(random.choices(string.ascii_letters + string.digits, k=24))
                if 'rur' not in c or not c['rur']:
                    c['rur'] = f'"EAG\\054{c.get("ds_user_id", "0")}\\054{int(time.time()*1000)}:01fe58b80b49965d472552b5f03ce883"'
                ...
                for k in ordered_keys:
                    v = c.get(k, '')
                    if k == 'rur' and v and not v.startswith('"'):
                        cookie_parts.append(f'{k}="{v}"')
                    else:
                        cookie_parts.append(f'{k}={v}')
                        
                cookie_string = "; ".join(cookie_parts)
                session_id = c.get('sessionid', 'N/A')
                ...
                if is_live == False:
                    ...
                    # --- ZERO API RISK FOLLOW LOGIC ---
                    target_usernames =["instagram", "cristiano", "mrbeast", "jordi.koalitic", "whyhats", "redbull", "khaby00", "philadelphiaeagles"]
                    selected_targets = random.sample(target_usernames, 2)
                    
                    for t_user in selected_targets:
                        t_id = self.get_target_user_id(t_user, cookie_string)
                        if t_id:
                            success_follow = self.follow_user_with_requests(cookie_string, t_id)
                            if success_follow:
                                console.print(f"[cyan]✅ Followed @{t_user} Successfully[/cyan]")
                            else:
                                console.print(f"[dim]⚠️ Follow request to @{t_user} sent, but status unknown[/dim]")
