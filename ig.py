...
class HotmailService:
    lock = threading.Lock()
    emails_list =[] # Will store dictionaries: {'line': "...", 'uses': 0}
        ...
        with cls.lock:
            if not cls.emails_list:
                ...
                if not os.path.exists(hotmail_path):
                    hotmail_path = "/sdcard/prial/hotmails.txt"
                
                if os.path.exists(hotmail_path):
                    with open(hotmail_path, "r", encoding="utf-8") as f:
                        lines = f.read().splitlines()
                        # Load emails with a usage counter set to 0
                        cls.emails_list =[{'line': line, 'uses': 0} for line in lines if line.strip()]
    ...
    @classmethod
    def get_next_email(cls):
        with cls.lock:
            if cls.emails_list:
                # Pop the email from the FRONT of the list
                current_email = cls.emails_list.pop(0)
                line_data = current_email['line']
                
                # Increment the usage count
                current_email['uses'] += 1
                use_count = current_email['uses']
                console.print(f"[dim]ℹ️ This Hotmail has been used {use_count}/5 times.[/dim]")
                
                # If it has reached 5 uses, we DON'T add it back to the list
                if current_email['uses'] >= 5:
                    # Append to used_hotmails.txt
                    try:
                        used_hotmail_path = "used_hotmails.txt"
                        if not os.path.exists(used_hotmail_path) and os.path.exists("/sdcard/prial/"):
                            used_hotmail_path = "/sdcard/prial/used_hotmails.txt"

                        with open(used_hotmail_path, "a", encoding="utf-8") as f:
                            f.write(line_data + "\n")
                    except Exception as e:
                        console.print(f"[red]❌ Error saving to used_hotmails.txt: {e}[/red]")
                else:
                    # Not yet 5 uses? Add it to the BACK of the queue to prevent multi-threading race conditions
                    cls.emails_list.append(current_email)

                # Update hotmails.txt with the new current queue order
                try:
                    hotmail_path = "hotmails.txt"
                    if not os.path.exists(hotmail_path) and os.path.exists("/sdcard/prial/hotmails.txt"):
                        hotmail_path = "/sdcard/prial/hotmails.txt"
                    
                    with open(hotmail_path, "w", encoding="utf-8") as f:
                        f.write("\n".join([item['line'] for item in cls.emails_list]))
                except Exception as e:
                    console.print(f"[red]❌ Error updating hotmails.txt: {e}[/red]")

                return line_data, use_count
            return None, 0
    ...
    def __init__(self):
        ...
        self.client_id = None
        self.ig_email = None # New alias email for Instagram

    def setup(self, line_data, use_count):
        try:
            ...
            if len(parts) >= 4:
                ...
                # --- ALIAS TRICK TO BYPASS 'email_is_taken' ---
                user_part, domain_part = self.email.split('@', 1)
                if use_count > 1:
                    # Generate a random suffix e.g., email+a8x9j@hotmail.com
                    random_suffix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=5))
                    self.ig_email = f"{user_part}+{random_suffix}@{domain_part}"
                else:
                    self.ig_email = self.email
    ...
    def get_code(self, retries=10, wait_seconds=5):
        url = "https://tools.dongvanfb.net/api/get_code_oauth2"
        # Always use original email 'self.email' for fetching code via Dong Van API
        payload = {
            "email": self.email,
...
class InstagramCreator:
    ...
    def setup_hotmail(self):
        result = HotmailService.get_next_email()
        if result[0] is None:
            console.print("[red]❌ No more Hotmail accounts available in hotmails.txt[/red]")
            return None
            
        line_data, use_count = result
        self.hotmail_service = HotmailService()
        if self.hotmail_service.setup(line_data, use_count):
            console.print(f"[green]✅ Using Hotmail: {self.hotmail_service.ig_email}[/green]")
            return self.hotmail_service.ig_email
        return None
    ...
    def check_email_availability(self, email):
        ...
        try:
            ...
            if '"available":true' in response_text:
                console.print("[green]✅ Email is available[/green]")
                return True
            else:
    ...
    def create_instagram_account_with_hotmail(self):
        email_address = self.setup_hotmail()
        if not email_address:
            return False
        ...
        console.print("[blue]⏳ Waiting for OTP via Dong Van API...[/blue]")
        code = self.hotmail_service.get_code()
        
        if not code:
            console.print("[red]❌ Failed to get OTP from API[/red]")
            return False
