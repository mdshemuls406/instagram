    ...
    def login(self):
        try:
            ...
            for inp in soup.find_all('input', type='hidden'):
                if inp.get('name') not in payload:
                    payload[inp.get('name')] = inp.get('value', '')
                    
            r2 = self.session.post(login_url, data=payload, headers={"Referer": login_url}, timeout=15)
            
            if "logout" in r2.text.lower() or "dashboard" in r2.url.lower() or "login" not in r2.url:
                console.print(f"[green]✅ Successfully logged into Embux.io as {self.username}[/green]")
                return True
            else:
                console.print("[red]❌ Embux login failed. Check username/password.[/red]")
                return False
        except Exception as e:
...
class InstagramCreator:
    ...
    def create_instagram_account_with_embux(self):
        with InstagramCreator._embux_task_lock:
            ...
            # If it's not the first thread, reload the main task page before accessing execute
            if not is_first:
                embux.reset_task()
