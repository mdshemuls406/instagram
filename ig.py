...
class EmbuxService:
    ...
    def login(self):
        try:
            ...
            if "login" not in r2.url:
                ...
                console.print("[yellow]⚠️ Embux login URL did not redirect, but attempting to proceed...[/yellow]")
                return True
        except Exception as e:
    ...
    def reset_task(self):
        try:
            console.print("[dim]🔄 Resetting/Reloading Embux Task Page...[/dim]")
    ...
    def get_email(self):
        try:
            ...
            match = re.search(r'[a-zA-Z0-9._-]+@(hotmail\.com|outlook\.com)', r.text, re.IGNORECASE)
            if match:
                self.email = match.group(0)
                # Capture all 6 digit numbers before requesting OTP to avoid false positives
                self.baseline_codes = set(re.findall(r'\b\d{6}\b', r.text))
                console.print(f"[green]✅ Retrieved email from Embux: {self.email}[/green]")
                return self.email
            else:
                console.print("[red]❌ Could not find email on Embux. Make sure task is available.[/red]")
                return None
        except Exception as e:
...
class InstagramCreator:
    ...
    def create_instagram_account_with_embux(self):
        # Embux allows only one task at a time per account. Sequential locking is required.
        with InstagramCreator._embux_task_lock:
            ...
            # Reset task for the next thread
            embux.reset_task()
    ...
    def run(self):
        try:
            ...
            # If Embux is selected, read credentials
            if email_type == "embux":
